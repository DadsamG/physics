...
function World:new(...)
   ...
   function w.collide(obja, objb, coll_type, ...)
      local function run_coll(obj1, obj2, ...)
         if obj1[coll_type] ~= nil then
            ...
            if type(e) == 'function' then w.collide_events[#w.collide_events+1] = e end
         end
      end
   ...
   function w.enter(a, b, ...) return w.collision(a, b, 'enter', ...) end
   function w.exit(a, b, ...) return w.collision(a, b, 'exit', ...) end
   function w.preSolve(a, b, ...) return w.collision(a, b, 'preSolve', ...) end
   function w.postSolve(a, b, ...) return w.collision(a, b, 'postSolve', ...) end

   function w.collision(a, b, ...)
      -- objects that hit one another can have collide methods by default used as postSolve callback
...
-- inputs: x1, y1, x2, y2: floats, the x and y coordinates of two points
-- outputs: colls: table, all colliders in bounding box
